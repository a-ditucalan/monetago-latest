...
const CorporateEntity = () => {
  ...
  const entities = Object.keys(orgData.orgEntities).map(key => {
    return orgData.orgEntities[key].filter(orgType => {
      if (finalSelctedOrg.orgId === orgType.orgId) {
        return orgType
      } else {
        return false
      }
  ...
  const finalEntities = entities.filter(entity => {
    if (entity.length !== 0) {
      return entity
    } else {
      return false
  ...
  const innerViewInfoTab = 'Information Details'
  const innerContactTab = 'Contact Person'
  const innerAddSchemeTab = 'Add Scheme'
  const innerEditSchemeTab = 'Edit Scheme'
  const innerViewSchemeTab = 'Scheme Details'
  ...
  const onChangeCountry = value => {
    setOrgType({ [orgType.legalEntityBase.tel1]: value })
  }
          ...
          <div className="corporate-entity-block-one-legal-info">
            ...
            <p className="corporate-identifier content-subtitle">
              ...
              <span className="corporate-identifier-text content-type">
                {finalEntities[0][0].corpIdentifiers}
              </span>
            </p>
            <p className="tel-one corporate-icon main-info">
              {finalEntities[0][0].tel1}
            </p>
            <p className="fax corporate-iconmain-info main-info">
              {finalEntities[0][0].tel2}
            </p>
            <p className="company-address corporate-icon address-primary">
              {finalEntities[0][0].registeredAddress}
              {/* {finalEntities[0][0].registeredAddress.floor}{' '}
            {finalEntities[0][0].registeredAddress.buildingNumOrName}{' '}
            {finalEntities[0][0].registeredAddress.street}{' '}
            {finalEntities[0][0].registeredAddress.area}{' '}
            {finalEntities[0][0].registeredAddress.city}{' '}
            {finalEntities[0][0].registeredAddress.state}{' '}
            {finalEntities[0][0].registeredAddress.postalCode}{' '}
            {finalEntities[0][0].registeredAddress.country}{' '} */}
            </p>
            <p className="corporate-email corporate-icon main-info">
              {finalEntities[0][0].primaryEmail}
            </p>
            <p className="corporate-domain corporate-icon main-info">
              {finalEntities[0][0].domain}
            </p>
          </div>
          <div className="corporate-entity-block-one-legal-subinfo">
            <p className="corporate-title">Organization Name</p>
            <p className="coporate-type">{finalEntities[0][0].networkRole}</p>
            ...
            <div className="business-unit-wrapper">
              ...
              <div className="business-unit-box">
                <p className="unit-name">
                  {finalEntities[0][0].businessUnits.unitName}
                </p>
